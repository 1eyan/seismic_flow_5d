...
class DiTBlockTrace(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, rope_base: float = 10000.0):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = TraceAxisAttention2D(dim=hidden_size, num_heads=num_heads, rope_base=rope_base)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.mlp = Mlp(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu, drop=0.1)
...
class SeisDiTRope(torch.nn.Module):
    ...
    def __init__(
        ...
        attenL = []
        for i in range(num_layers):
            attenL.append(DiTBlockTrace(hidden_size=d_model, num_heads=nhead, rope_base=rope_base))
        self.attenL = torch.nn.ModuleList(attenL)
        self.norm_final = nn.LayerNorm(d_model, elementwise_affine=False, eps=1e-6)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
...
class SeisDiTRopeV2(torch.nn.Module):
    ...
    def __init__(
        ...
        self._printed_coord_summary = False
        #self.fourier_encoder=fourier_enoder.Seismic5DEncoder(coord_dim=4,max_freq=128,out_dim=d_model,num_bands=32,pe_type = pe_type)
        self.tokenizer = torch.nn.Conv2d(
            image_channels//2, n_channels, kernel_size=(1, 3), padding=(0, 1), bias=True
        )
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, condL=None, log_tau=None, time_axis=None, training=False):
        ...
        if condL is not None:
            rx, ry, sx, sy = condL
            if self.coord_config and self.coord_config.get("rope_input_coord_unit") == "normalized":
                # Dataset already normalized — pass through directly, no centering
                pos_emb = torch.stack([rx, ry, sx, sy], dim=-1)  # (B, H, 4)
            else:
                # Legacy: center around source mean
                x_mean = sx.mean(dim=-1, keepdim=True)
                y_mean = sy.mean(dim=-1, keepdim=True)
                sx = sx - x_mean
                sy = sy - y_mean
            ...
            # One-time coord summary on first forward
            if not self._printed_coord_summary and self.coord_config:
                self._printed_coord_summary = True
                lines = [
                    "[SeisDiTRopeV2] Coordinate config:",
                    f"  coord_norm_mode={self.coord_config.get('coord_norm_mode')}",
                    f"  rope_input_coord_unit={self.coord_config.get('rope_input_coord_unit')}",
                    f"  coord_range={self.coord_config.get('coord_range')}",
                    f"  omega={self.coord_config.get('omega')}",
                    f"  skip_centering={'normalized' == self.coord_config.get('rope_input_coord_unit')}",
                ]
                print("\n".join(lines))
        ...
        for _ai, atten in enumerate(self.attenL):
            x = atten(x, fourier_emb, rope_pos=rope_pos)
