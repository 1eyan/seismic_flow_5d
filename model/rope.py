...
class SegmentedRoPEExpCached(nn.Module):
    def __init__(self, D, N, n_pos, min_log=0.0, max_log=4.0, mapper="linear", hidden=128, use_time=False, base=10000.0):
        super().__init__()
        assert D % N == 0
        d_seg = D // N
        assert d_seg % 2 == 0
        ...
        inv_freq = 1.0 / (self.base ** (torch.arange(0, self.half, dtype=torch.float32) * 2.0 / self.d_seg))
